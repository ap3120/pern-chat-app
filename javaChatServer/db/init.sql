...
create table messages (message_id serial primary key, content varchar(1000), sender_id integer references users(user_id), sender_username varchar(50), receiver_id integer, receiver_username varchar(50), chat_id integer references chats(chat_id), send_at timestamp);
